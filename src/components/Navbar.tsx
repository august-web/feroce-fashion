  ...
  return (
    <>
      <nav className="sticky top-0 z-50 border-b border-line bg-white">
        <div className="mx-auto flex h-12 sm:h-14 md:h-16 max-w-7xl items-center justify-between px-2.5 sm:px-4 md:px-8">          {/* Left — Desktop nav links */}
          <div className="hidden items-center gap-6 lg:gap-8 md:flex">
            ...
            <Link href="/about" className="text-[11px] font-sans uppercase tracking-luxury text-navy/70 hover:text-gold transition-colors py-2">About</Link>
            <Link href="/faq" className="text-[11px] font-sans uppercase tracking-luxury text-navy/70 hover:text-gold transition-colors py-2">FAQ</Link>
            <Link href="/contact" className="text-[11px] font-sans uppercase tracking-luxury text-navy/70 hover:text-gold transition-colors py-2">Contact</Link>
          </div>
          {/* Center — Logo */}
      ...
      <div className={'fixed top-0 left-0 z-[70] h-full w-[80vw] max-w-sm bg-white shadow-xl transition-transform duration-300 ease-out md:hidden ' + (menuOpen ? 'translate-x-0' : '-translate-x-full')}>
        ...
        <div className="px-6 py-4 border-t border-line">
          <p className="label mb-3 text-navy/40">Info</p>
          <ul className="space-y-1">
            <li><Link href="/about" onClick={() => setMenuOpen(false)} className="flex h-12 items-center text-[11px] uppercase tracking-luxury text-navy/70 hover:text-gold transition-colors border-b border-line/50">About</Link></li>
            <li><Link href="/faq" onClick={() => setMenuOpen(false)} className="flex h-12 items-center text-[11px] uppercase tracking-luxury text-navy/70 hover:text-gold transition-colors border-b border-line/50">FAQ</Link></li>
            <li><Link href="/contact" onClick={() => setMenuOpen(false)} className="flex h-12 items-center text-[11px] uppercase tracking-luxury text-navy/70 hover:text-gold transition-colors border-b border-line/50">Contact</Link></li>
          </ul>
        </div>
        <div className="px-6 py-4 border-t border-line">
          <button onClick={() => { setMenuOpen(false); setSearchOpen(true) }} className="flex h-12 items-center text-[11px] uppercase tracking-luxury text-navy hover:text-gold transition-colors w-full"><Search size={16} strokeWidth={1.5} className="mr-3" />Search</button>
        </div>
      </div>
      {/* Search Modal */}

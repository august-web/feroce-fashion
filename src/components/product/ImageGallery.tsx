...
import type { ProductVariant } from '@/data/seed'

interface ImageGalleryProps {
  images: string[]
  modelImages?: string[]
  productName: string
  /** When provided, gallery shows these variant-specific images */
...
export function ImageGallery({ images, modelImages, productName, activeVariant }: ImageGalleryProps) {
  ...
  // When a color variant is selected, show its model images in the gallery
  const displayImages = activeVariant
    ? activeVariant.modelImages.length > 0
      ? [activeVariant.images[0], ...activeVariant.modelImages]
      : activeVariant.images
    : modelImages && modelImages.length > 0
      ? [images[0], ...modelImages]
      : images.length > 0
        ? images
        : ['/images/products/Denim De Ville Collection/Blue & Gold/Denim De Ville Collection --Blue & Gold.jpg']
          ...
          {displayImages.map((img, i) => (
            <button
              ...
              className={"relative h-16 w-16 flex-shrink-0 overflow-hidden border-2 transition-all duration-300 sm:h-20 sm:w-20 " +
                (i === activeIdx ? 'border-navy' : 'border-line hover:border-navy/30')}
            >
              <img
                src={img}
